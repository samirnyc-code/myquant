...
| 20 | Negative-gamma SCHEDULED-EVENT days: reduce size / widen wings / stand aside — EM breaks concentrate here | archive read (EM-hit False cluster) | 08-07 | **measuring (n=1)** | REFINED: it's the POS/NEG gamma split that matters, not "event day." 08-07 NFP (payrolls −23k vs +83k) was POSITIVE gamma + no in-range flip → EM HELD, we netted +$1,367 trading EOD at 08:30. Rule → "event + neg-gamma = stand aside; event + pos-gamma + no in-range flip = EM holds, sell it." Gate on gamma regime, not the calendar alone |
