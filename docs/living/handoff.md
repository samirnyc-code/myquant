...
| 15 | ETH tick trove | ✅ LOCKED IN (S116-B 9/13) | **`data/ticks_continuous_eth/` = 301 days full-session ETH, 2025-07-16 → 2026-09-11, 307.9M ticks, back-adjusted + roll-guarded + CONTINUOUS.** Built from NT `.ncd` (only ETH source — Massive cancelled/revoked; the 5yr trove is RTH-ONLY, verified every year 08:30-15:15, NOT ETH). Validated (`validate_eth_store.py`): 230 match Massive vol ≤0.1% / 51 NT-more-complete / 13 NT-light / 7 eth-only / 0 struct. LOCK: manifest `docs/reference/eth_store_manifest.csv` (sha1/day; `eth_store_manifest.py --check` detects erasure) + off-dir backup `C:\eth_trove_backup\`. Rebuild if lost: `backfill_eth_from_nt.py` (while NT .ncd still holds it). ⚠ FINDING: recent RTH trove (46 days 2026-07+) is volume-UNDERCOUNTED by the same dedup bug in `ingest_nt_ticks.py` — NOT fixed (user: don't touch trove). Full 5yr ETH needs Massive re-subscription |
