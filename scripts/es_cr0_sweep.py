"""Stop/target sweep + monthly distribution for the CR-0DTE fade (S73 night).
...
def entries():
    ...
    levels = {r.date: r.cr0 for r in lv.itertuples() if np.isfinite(r.cr0)}
...
def main():
    ents = entries()
    print(f"CR-0DTE fade entries (from below, any regime): {len(ents)}")
