...
OUT = OUT_DIR / "es_regime_recheck_three.html"
DAYS = ["20251226", "20260519", "20260521"]

NOTES = {
 "20251226": """A choppy holiday-week session. <b>Bear</b> from bar 17 to 18, again from bar
  31 to 36, then three Bull attempts &mdash; 43, 53 and 67 &mdash; each giving way inside
  a few bars. My first attempt at the rule collapsed all of this into one Bear from 21 to
  53; it no longer does, because bar 16's outside bar made new ground at both ends.""",
 "20260519": """Three bull attempts in the morning (bars 15, 21, 31), each ChoCh'd within two
  bars, then Range until the <b>bar 52</b> BOS and Bear to the close. The earlier attempt
  turned the bar-15 Bull into a bar-16 Bear; bar 13's outside bar extends both ways, so it
  stands as it was.""",
 "20260521": """Mostly range &mdash; 70 of 81 bars carry no trend. <b>Bull opens at bar 28</b>
  off the break of bar 23's high (7492.25), with bar 21's session low and bar 25's higher
  low behind it, which is how you read it.""",
}
...
HTML = f"""<title>Three Sessions Rechecked</title>
...
<div class="wrap">
  ...
  <h1>Three Sessions Rechecked</h1>
  <p class="standfirst">These are the three sessions my first attempt at the outside-bar
  rule disturbed. Under the rule as it now stands they are <b>byte-identical</b> to the
  version you already reviewed &mdash; the earlier changes are gone. Shown here so you can
  confirm that rather than take my word for it.</p>
  ...
  <section class="flags">
    <h2>What the rule does now</h2>
    <p>An end of an outside bar is dropped only when it fails to <b>extend</b> the
    structure &mdash; when it does not run past the last swing of its own kind.</p>

    <div class="flag">
      <div class="where">02-13 b6<small>changed</small></div>
      <div>
        <p>The high (6966.25) never reached bar 4's 6973.75, so it is an overshoot inside
        the bar and is dropped; the low made new ground and stands. That restores the
        b3&ndash;b4&ndash;b6 structure and the <b>bar 9 BOS</b>.</p>
      </div>
    </div>

    <div class="flag">
      <div class="where">05-21 b21<small>unchanged</small></div>
      <div>
        <p>The low set a new session low <i>and</i> the high cleared bar 18's 7491.25 by
        0.75. Both ends made new ground, so both swings stand and the Bull still opens at
        <b>bar 28</b> off the break of bar 23's high &mdash; as you read it.</p>
        <p>The same "both ends keep" case leaves 12-26 and 05-19 untouched.</p>
      </div>
    </div>
  </section>
{"".join(sections)}
