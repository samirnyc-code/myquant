...
OUT = OUT_DIR / "es_regime_week_0518.html"
DAYS = ["20260518", "20260519", "20260520", "20260521", "20260522"]

NOTES = {
 "20260518": """The widest session of the week: 81 points from the bar 6 high (7515.50) to the bar 66
  low (7434.75). Five separate trends, six ChoCh &mdash; a genuinely two-sided day, with
  neither side holding structure for more than 14 bars.""",
 "20260519": """The day we worked through under the old rules, where bars 33&ndash;51 were the
  transition in question. Under the PDF rules it stays Range until the <b>bar 52</b> BOS,
  then Bear to the close with four more BOS. The morning shows three bull attempts
  (bars 15, 21, 31), each ChoCh'd within two bars. Worth checking against your read.""",
 "20260520": """A trend day up. Bull from bar 13, one interruption at bars 36&ndash;45, then Bull
  again from bar 45 to a close of 7509.25, ten points under the high. The interruption
  includes the bar 37 case above.""",
 "20260521": """Mostly range: 70 of 81 bars carry no trend. The longest trend is Bull
  28&ndash;34, six bars; the rest are breaks that ChoCh'd almost at once, including the one-bar
  Bull at 63 and the zero-length pair at bar 75.""",
 "20260522": """Clean three-act session: a short Bear from bar 7, a 49-bar Bull from bar 18 to the
  7585.25 high, then Bear from bar 68 until a ChoCh at bar 78.""",
}
...
for D in days:
    ...
    <a class="row" href="#d{D["s"]}" role="listitem">
      <span class="day"><b>{D["date"]:%a}</b> {D["date"].day} May</span>
...
HTML = f"""<title>ES Regime Week of 18 May</title>
...
<div class="wrap">
  <p class="eyebrow">Regime tracker &middot; ES 5-minute &middot; 18&ndash;22 May 2026</p>
  <h1>ES Regime Week of 18 May</h1>
  <p class="standfirst">Five sessions under the Market Structure rules. Wednesday and Friday
  held long trends; Monday traded both sides hard; Thursday barely left Range. Each row is
  one session, 81 bars from 08:30 to 15:10 &mdash; click a row to jump to its chart.</p>
  ...
  <section class="flags">
    <h2>Worth a look</h2>
    <p>All three are rule-correct as the rules stand. Two are the outside-bar family we
    parked; one is a judgment call about the rules themselves.</p>

    <div class="flag">
      <div class="where">Mon b4<small>Thu b75</small></div>
      <div>
        <p><b>Zero-length trends on outside bars.</b> On Monday, bar 3 sets a lower high
        (7492.75) and bar 4, a bullish outside bar, trades low-first through the bar 2 low
        (7483.75): BOS down, Bear. The same bar then takes out 7492.75: ChoCh, back to Range.
        Thursday's bar 75 is the mirror image: a bearish outside bar pokes 1.25 points above a
        flat 7527.75 top, then undercuts the higher low.</p>
        <p>Both trends start and end inside a single bar, so neither appears as a band on the
        chart. Same family as the parked 02-13 b76 case. Bar-only, so no fix proposed.</p>
      </div>
    </div>

    <div class="flag">
      <div class="where">Wed b37<small>ref 7498.75</small></div>
      <div>
        <p><b>A range exit whose level broke before the range began.</b> The bear setup's
        reference low (7498.75) was already taken out at bar 34, while the market was still
        Bull. The bar 36 ChoCh returns to Range, and on bar 37 the setup fires at once, 13
        points below its own level. That is why the BOS line on Wednesday's chart sits above
        the bar.</p>
        <p class="ask">Should a range exit need the break to happen while in Range, or does
        structure completed during the prior trend count straight away? Your earlier rule
        &mdash; &ldquo;as soon as you discover a trend structure when in a transition period,
        that trend becomes the new state&rdquo; &mdash; says it counts, so I've left it.</p>
      </div>
    </div>
  </section>
{"".join(sections)}
