...
OUT = OUT_DIR / "es_regime_0213.html"
SLUG = "20260213"
...
HTML = f"""<title>ES Regime 13 February 2026</title>
...
<div class="wrap">
  ...
  <h1>ES Regime 13 February 2026</h1>
  <p class="standfirst">The session the tracker was first validated against.
  A <b>Bull</b> structure from bar 9 runs to a ChoCh at bar 43, then eight bars of Range
  before <b>Bear</b> takes over at bar 51 and holds into the close.</p>
  ...
  <button class="shot" data-full="big" aria-label="Enlarge chart">
    <img id="big" src="data:image/png;base64,{img}" alt="ES regime chart for 13 February 2026">
  ...
  <div class="note">
    <p><b>Bar 9 now fires.</b> The structure is the document's 1-2-3: low at bar 3
    (6919.75), high at bar 4 (6973.75), higher low at bar 6 (6941.00) &mdash; and bar 9
    takes out bar 4's high at 6975.00.</p>
    <p style="margin-top:10px">It was missed because bar 6 is an outside bar, which mywedge
    marks as both a swing high (6966.25) and a swing low (6941.00). Those two entries split
    one pullback in half, so the higher-low test compared bar 6 against <b>bar 5</b>
    (6942.75) rather than against bar 3, and read a higher low as a lower low.</p>
    <p style="margin-top:10px">The rule now: an end of an outside bar is dropped when it does
    not <b>extend</b> the structure &mdash; when it fails to run past the last swing of its
    own kind. Bar 6's high never reached bar 4's 6973.75, so it is an overshoot inside the
    bar, not a turning point. Its low did make new ground, so it stands. When both ends
    extend the bar genuinely moved in both directions and both swings are kept &mdash; which
    is why this change moves bar 9 and nothing else across the twelve sessions on disk.</p>
  </div>
</div>
