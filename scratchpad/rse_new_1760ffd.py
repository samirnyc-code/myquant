...
OUT = WT_ROOT / "scratchpad" / "second_entries_NEW_1760ffd_20260724.csv"
...
def phase_transitions(H, L, n, tP, tbar, trace=None):
    ...
    def add_pivot(bar, side, emit):
        ...
        piv.append(p)
        # 1760ffd: structure trackers run in EVERY mode (capture counter-trend minor
        # structure forming inside a trend, for the immediate-flip check in terminate)
        if side == "H":
            lsh = p
    ...
    def start_trend(up, b, px):
        ...
        # 1760ffd: fresh tracker scope for the new trend
    ...
    def terminate(b, px):
        ...
        # 1760ffd IMMEDIATE FLIP: counter-trend minor structure formed inside the dying
        # trend may already satisfy the opposite race at this same tick -> skip neutral.
        # Partner must have formed AFTER the candidate (struct_*.bar > ls*.bar).
        ...
        # 1760ffd: prevH/prevL and the leg (d/leg_px/leg_bar) are NOT reset at termination.
        if flip_up or flip_dn:
            start_trend(flip_up, b, px)   # uses still-populated lsh/lsl/struct_*/hi_p/lo_p
        else:
            hi_p = lo_p = lsh = lsl = None; has_hl = has_lh = False
            struct_hl = struct_lh = None
