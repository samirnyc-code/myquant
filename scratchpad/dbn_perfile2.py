...
c=db.Historical(k); job='GLBX-20260721-JRSPF47X5J'
out=Path(r'data\databento')/job
fs=c.batch.list_files(job); n=len(fs)
print(f'{n} files, target {out}',flush=True)
for i,f in enumerate(fs,1):
    fn=f['filename']; sz=f.get('size',0); dest=out/job/fn
    ...
    for a in range(5):
        try:
            ...
            print(f'[{i}/{n}] retry{a} {fn}: {str(e)[:50]}',flush=True); time.sleep(15)
